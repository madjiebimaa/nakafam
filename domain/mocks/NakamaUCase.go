...
func (n *NakamaUCase) GetByName(ctx context.Context, name string) (_nakamaRes.NakamaBase, error) {
	ret := n.Called(ctx, name)

	var r0 _nakamaRes.NakamaBase
	if ref, ok := ret.Get(0).(func(context.Context, string) _nakamaRes.NakamaBase); ok {
		r0 = ref(ctx, name)
	} else {
		r0 = ret.Get(0).(_nakamaRes.NakamaBase)
	}
